...
# Jongyeon Yoon
# June 7, 2024
# Added inferencing with clinical dMRI's FODs
# Set Up
...
if __name__ == '__main__':
    ...
    ### Add dMRI
    parser.add_argument('dmri_file', metavar='/in/file.nii.gz', help='path to the input NIFTI Diffusion MRI file')
    parser.add_argument('bvec_file', metavar='/in/file.bvec', help='path to the input b-vector file')
    parser.add_argument('bval_file', metavar='/in/file.bval', help='path to the input b-value file')
    ###
    ...
    assert os.path.exists(args.bvec_file), 'Input b-vector file {} does not exist. Aborting.'.format(bvec_file)
    echo('Input bvec file:\t\t{}'.format(bvec_file))
    ...
    echo('--Moving T1w MRI to working directory--')
    if not os.path.exists(os.path.join(work_dir, 'T1.nii.gz')):
        convert_t1_cmd = 'mrconvert {} {}'.format(t1_file, os.path.join(work_dir, 'T1.nii.gz'))
        run(convert_t1_cmd)
    else:
        echo('T1.nii.gz already exists, skipping.')
    
    echo('--Moving dMRI to working directory--')
    if not os.path.exists(os.path.join(work_dir, 'dmri.nii.gz')):
        convert_dmri_cmd = 'mrconvert {} {}'.format(dmri_file, os.path.join(work_dir, 'dmri.nii.gz'))
        run(convert_dmri_cmd)
    else:
        echo('dmri.nii.gz already exists, skipping.')
    
    echo('--Moving bvec to working directory--')
    if not os.path.exists(os.path.join(work_dir, 'dmri.bvec')):
        cp_bvec_cmd = 'cp {} {}'.format(bvec_file, os.path.join(work_dir, 'dmri.bvec'))
        run(cp_bvec_cmd)
    else:
        echo('dmri.bvec already exists, skipping.')
    
    echo('--Moving bval to working directory--')
    if not os.path.exists(os.path.join(work_dir, 'dmri.bval')):
        cp_bval_cmd = 'cp {} {}'.format(bval_file, os.path.join(work_dir, 'dmri.bval'))
        run(cp_bval_cmd)
    else:
        echo('dmri.bval already exists, skipping.')
    ...
    echo('<<<< Preparing dMRI & T1w MRI >>>>')
    t1_cmd = 'source {}/bin/activate ; bash {} {} {} {} {}'.format(VENV_DIR, os.path.join(SRC_DIR, 'prep_DT1.sh'), work_dir, slant_file, wml_dir, num_threads)
    run(t1_cmd)
    ...
    fod_img = nib.load(os.path.join(work_dir, 'T1_fod_mni_2mm.nii.gz')).get_fdata()
    fod = torch.FloatTensor(np.expand_dims(np.transpose(fod_img, axes=(3, 0, 1, 2)), axis=0))
    
    fod_ten = torch.cat((ten, fod), dim=1)
    ...
    cnn      = DetConvProj(168, 512, kernel_size=7)
    rnn      = DetRNN(512, fc_width=512, fc_depth=4, rnn_width=512, rnn_depth=2)
    cnn.load_state_dict(torch.load(os.path.join(MODEL_DIR, 'dt1_cnn.pt'), map_location=torch.device('cpu')))
    rnn.load_state_dict(torch.load(os.path.join(MODEL_DIR, 'dt1_rnn.pt'), map_location=torch.device('cpu')))
    

    ## Inference (image-level, CNN part)


    echo('<<<< Performing inference (image) >>>>')
    if not os.path.exists(os.path.join(work_dir, 'inference_mni_2mm.nii.gz')):
        ten = ten2features(fod_ten, cnn, device)
